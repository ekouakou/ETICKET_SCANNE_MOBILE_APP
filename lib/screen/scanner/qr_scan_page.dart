...
import '../../widgets/modal/custme_dialog.dart'; // Import du service utilisateur
...
class _QRScanPageState extends State<QRScanPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _sendScanResult(String? code) async {
    ...
    if (response.statusCode == 200) {
      ...
      _isDialogOpen = true;
      showCustomDialog(context, "Échec du scan", icon: Icons.error, iconColor: Colors.red);
    }
  }



  void showCustomDialog(BuildContext context, String message, {IconData? icon, Color? iconColor}) {
    showDialog(
      context: context,
...
// 1. Avec une icône d'erreur
// showCustomDialog(context, 'Échec du scan', icon: Icons.error, iconColor: Colors.red);

// 2. Avec une icône d'information
//showCustomDialog(context, 'Opération réussie', icon: Icons.info, iconColor: Colors.blue);

// 3. Sans icône
  //showCustomDialog(context, 'Aucune icône ici');
